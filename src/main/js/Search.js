...
class Search extends Component {
    	...
    	setTimeout(() => {
	        ...
	        .then(data => {
	            ...
	            data.results.forEach(movie => {
	            	...
	            	// Second array, 'movies', created solely to keep track of
                // tmdbId
	            	// (PrimeReact 'AutoComplete' component does not allow storing
                // objects)
	                movies.push({title: titleWithYear, tmdbId: movie.id});
	            })
    ...
    handleSelectMovie(event) {
    	...
    	this.setState({tmdbId: selectedMovie[0].tmdbId});
    }
    ...
    render() {
        return (
            <div>
                ...
                <br /><br />
                { (this.state.tmdbId > 0) ? 
                		<Movie tmdbId={this.state.tmdbId} 
                               updateMovies={null} 
